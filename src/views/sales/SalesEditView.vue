<template>
  <div>
    <div class="mb-6 flex justify-between items-center">
      <h1 class="text-2xl font-bold">Editar Venda</h1>
      <router-link 
        to="/sales" 
...
export default defineComponent({
  ...
  setup() {
    ...
    const fetchSale = async () => {
      loading.value = true;
      try {
        await salesStore.fetchSaleById(saleId);
        const sale = salesStore.getCurrentSale;
        if (sale) {
          form.seller_id = sale.seller_id;
          form.amount = sale.amount;
          form.sale_date = sale.sale_date;
        }
      } catch (err: any) {
        error.value = err.response?.data?.message || 'Erro ao carregar dados da venda';
    ...
    const handleSubmit = async () => {
      ...
      try {
        await salesStore.updateSale(saleId, form);
        showToast('Venda atualizada com sucesso', 'success');
        router.push('/sales');
      } catch (err: any) {
        error.value = err.response?.data?.message || 'Erro ao atualizar venda';
